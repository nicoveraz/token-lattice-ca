...
def _verdict(cache, pair):
    ...
    for m, pr in sorted(pair["pairs"].items()):
        ...
        floored = (max(mA, mB) < HEADROOM and mr < HEADROOM) or (min(mA, mB) > 1 - HEADROOM and mr > 1 - HEADROOM)
        ...
        res["models"][m] = row
        if not row["readable"]:
            res["excluded"].append(dict(model=m, why="no arm moved beyond tolerance" if not moved
                                        else "floored or ceilinged"))
    ...
    if res["K3_vacuity"]:
        p.append("K3 FIRES: no model has a pair where either arm moves phi beyond its own "
