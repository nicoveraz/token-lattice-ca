...
| `gatecheck.leverage` | Does this quantity have room to carry the claim being made about it? Dynamic range, **unit resolution** (`resolves_units`: is the spread bigger than the spread noise alone would give?), noise gates, directionality, distinct units — composed into `NOT_DECIDABLE` with the binding reason attached | textca F80/F88/F93/F94/F96: one defect class — a statistically shaped criterion applied to a quantity with no room to vary — caught by hand six times, three of them *after* preregistration. `resolves_units` descends from a seventh, inside this package: a `span >= 2 x SD` gate that pure noise passes ~93% of the time at k=10 |
