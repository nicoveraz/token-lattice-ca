...
token-space Lyapunov exponent crosses from negative to positive between steps $512$ and
$1000$ and stays there, at two lattice sizes ($d{=}2.74$ and $2.78$; all four members of a
pre-registered family survive Benjamini--Hochberg correction), with seed-to-seed variance
collapsing $\sim\!3\times$ across the transition. We then \emph{delimit} the instrument, which
...
training the lattice is sub-critical---a perturbation dies---and after a transition between
steps $512$ and $1000$ it is super-critical and remains so for the rest of training.
...
survive} ($p_{\mathrm{BH}}\le2\times10^{-5}$). Measured against the settled plateau
(steps $2000/8000/143000$), $\lca$ moves $-0.0185\to+0.1683$ at $N{=}48$ ($d{=}2.74$,
$p{=}6.6\times10^{-5}$) and $-0.0307\to+0.1686$ at $N{=}96$ ($d{=}2.78$,
$p{=}8.6\times10^{-6}$).

\textbf{The curve is not a step, and we quote it accordingly.} It rises, overshoots at step
$1000$ by $14$--$22\%$, and settles. Quoting step $256$ against the step-$1000$ peak would
inflate the $N{=}48$ \dn{} effect from $d{=}3.69$ to $d{=}5.04$---taking the transition's peak
for its level. We therefore report against the plateau throughout. The overshoot itself
separates from the plateau in only one of four cells after correction, so we describe the
shape as non-monotone but do \emph{not} claim step $1000$ as a distinct developmental phase.

\textbf{Which quantity carries the claim.} $\lca$ is size-robust: the effect does not shrink
with a doubled lattice ($107\%$ retention) and the plateau \emph{level} is statistically
indistinguishable across sizes ($+0.1683$ vs.\ $+0.1686$, $p{=}0.91$). \dn{} is not: its gap
roughly halves and its plateau level differs decisively ($0.569$ vs.\ $0.306$,
$p{=}1.3\times10^{-8}$), even though the standardized effect is undiminished ($d$ $3.69\to3.91$)
...
after it they agree to a few percent. It was not pre-registered, but it replicates
independently at both lattice sizes.

\begin{figure}[t]\centering
\includegraphics[width=0.97\linewidth]{developmental.png}
\caption{The developmental transition. (A) $\lca$ crosses zero between steps $512$ and $1000$
and stays; the two lattice sizes lie on top of each other. (B) \dn{} shows the same shape but
